...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if cached_response:
            logger.info(f"[TELEGRAM_BOT] ✅ Ответ найден в кэше для запроса: {user_query[:50]}...")
            await processing_message.edit_text(cached_response, parse_mode="Markdown")
            return
        logger.info(f"[TELEGRAM_BOT] Кэш не содержит ответа, продолжаем обработку")
        ...
        if chunks == NOT_FOUND:
            logger.warning(f"[TELEGRAM_BOT] ❌ Не найдено релевантных чанков для запроса: {user_query[:50]}...")
        ...
        if isinstance(chunks, list):
            logger.info(f"[TELEGRAM_BOT] ✅ Найдено {len(chunks)} релевантных чанков")
            for i, chunk in enumerate(chunks):
                logger.debug(f"[TELEGRAM_BOT] Чанк {i+1}: source={chunk.get('source')}, score={chunk.get('score')}, text_length={len(chunk.get('text', ''))}")
        else:
            logger.error(f"[TELEGRAM_BOT] ❌ Неожиданный тип chunks: {type(chunks)}")
            response_text = format_response(
                AnalysisResponse(status="NOT_FOUND", clarification_question=None, result=None)
            )
            await processing_message.edit_text(response_text, parse_mode="Markdown")
        ...
        logger.info(f"[TELEGRAM_BOT] ✅ Анализ завершён, статус: {analysis_response.status}")

        # 4. Форматирование ответа
        logger.info(f"[TELEGRAM_BOT] Этап 4/6: Форматирование ответа")
        response_text = format_response(analysis_response)
        logger.debug(f"[TELEGRAM_BOT] Сформирован ответ длиной {len(response_text)} символов")
